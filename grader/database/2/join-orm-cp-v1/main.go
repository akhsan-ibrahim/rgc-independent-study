...
func (t Teacher) Join(db *gorm.DB) ([]Joined, error) {
	result := []Joined{}
	err := db.Table("teachers").Select("teachers.name AS teacher_name, schools.name AS school_name, classes.name AS class_name, lessons.name AS lesson_name").Joins("INNER JOIN schools ON schools.id = teachers.id").Joins("INNER JOIN classes ON classes.id = teachers.id").Joins("INNER JOIN lessons ON lessons.id = teachers.id").Scan(&result).Error
	if err != nil {
		return []Joined{}, err
	}
